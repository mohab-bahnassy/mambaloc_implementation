...
# Adapted for UWB regression tasks
...
@dataclass
class UWBRegressionOutput(ModelOutput):
...
class UWBRegressionModel(nn.Module):
    def __init__(
        ...
        # UWB data dimensions
        input_features = config.UWBRegressionModel.input.input_features  # 10 for UWB data
        output_features = config.UWBRegressionModel.input.output_features  
        self.output_mode = config.UWBRegressionModel.input.output_mode  # "sequence" or "last"
        ...
        self.dropout = nn.Dropout(config.UWBRegressionModel.input.get("dropout", 0.1))
    ...
    @property
    def d_model(self):
        """Property to access d_model for compatibility with original mohawk pattern."""
        return self.config.UWBMixerModel.input.d_model
